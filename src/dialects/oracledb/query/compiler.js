...
function QueryCompiler_OracleDb(client, builder) {
  QueryCompiler.call(this, client, builder);
}
inherits(QueryCompiler_OracleDb, QueryCompiler);

assign(QueryCompiler_OracleDb.prototype, {
  ...
  insert: function() {
    ...
    if (returning) {
      ...
      sql.returningSql = 'select ' + this.formatter.columnize(returning) +
        ' from ' + this.tableName +
        ' where ROWID in (' +
        sql.outParams.map(function (v, i) {
          return ':' + (i + 1);
        }).join(', ') + ')' +
        ' order by case ROWID ' +
        sql.outParams.map(function (v, i) {
...
QueryCompiler_OracleDb.prototype.first = QueryCompiler_OracleDb.prototype.select;

module.exports = QueryCompiler_OracleDb;
