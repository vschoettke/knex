...
module.exports = function(client, clientName, aliasName) {
  ...
  describe("Quoting " + clientName, function() {
    ...
    it("basic select", function() {
      testsql(qb().quote(true).select('*').from('users'), {
    ...
    it("adding selects", function() {
      testsql(qb().quote(true).select('foo').select('bar').select(['baz', 'boom']).from('users'), {
    ...
    it("basic wheres", function() {
      testsql(qb().quote(true).select('*').from('users').where('id', '=', 1), {
        mysql: {
    ...
    it("sub select where ins", function() {
      testsql(qb().quote(true).select('*').from('users').whereIn('id', function(qb) {
    ...
    it("sub select where ins no quoting", function() {
      testsql(qb().select('test').from('users').whereIn('id', function(qb) {
        qb.select('id').from('users').where('age', '>', 25).limit(3);
      }), {
        oracle: {
    ...
    it("sub select where ins with special words no quoting", function() {
      testsql(qb().select('test').from('tEsT').whereIn('id', function(qb) {
    ...
    it("raw wheres", function() {
      testsql(qb().quote(true).select('*').from('users').where(raw('id = ? or email = ?', [1, 'foo'])), {
        mysql: {
    ...
    it("allows for case-insensitive alias", function() {
      testsql(qb().quote(true).select(' foo   aS bar ').from('users'), {
    ...
    it("basic table wrapping", function() {
      testsql(qb().quote(true).select('foo').from('public.users'), {
    ...
    it('test basic create table with charset and collate ', function() {
      var tableSql = new client.SchemaBuilder().quote(true).createTable('users', function(table) {
        table.increments('id');
        table.string('email');
      });
    ...
    it('test basic create table with charset and collate no quoting', function() {
